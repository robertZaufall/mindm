...
`mindm/` hosts the platform connectors (`mindmanager.py`, `mindmanager_win.py`, `mindmanager_mac*.py`) with direct MindManager automation hooks, while `mindmap/` provides the higher-level document model plus serialization helpers (`serialization.py`, `export.py`, `import.py`). Reference material lives in `docs/` (Sphinx) and publishes via `docs/_build/html`. Usage snippets and sanity scripts sit in `examples/`, and build artifacts land in `dist/`. Tweak version bumps through `update_version.py`, and manage builds with the root `Makefile`.
...
- `python examples/test.py`: quick smoke run against a connected MindManager instance.  
