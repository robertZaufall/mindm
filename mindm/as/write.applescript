...
property guidMapping : {}
property pendingReferences : {}
property guidMappingDict : missing value
...
on handleCommand(argv)
	if (count of argv) is 0 then
		if SCRIPT_EDITOR_MODE then
			...
			set testResult to my handleCommand({"writeFragment", "{\"guid\": \"ba179bfd-6528-47ff-9ea2-88f07b741a08\", \"text\": \"Creating an AI startup\", \"subtopics\": [{\"guid\": \"7d1f6e5f-d76b-4e80-9a0f-f327562b3fb6\", \"text\": \"Vision & Strategy\", \"subtopics\": [{\"guid\": \"9c7a2f49-0ac8-4c9b-99c8-c5b0678d1839\", \"text\": \"Mission and Value\", \"subtopics\": [{\"guid\": \"63eadbaa-fb86-44b9-a770-1bbcf7d12ad1\", \"text\": \"Problem statement\"}, {\"guid\": \"cc649bd4-f57f-4835-8f69-e33f56ebf125\", \"text\": \"Value proposition\"}, {\"guid\": \"ccdb5ce8-e25d-4633-8b89-f052b5f5d034\", \"text\": \"Long term goals\"}]}, {\"guid\": \"1d527fd2-4220-45db-8ecd-9d99c5660204\", \"text\": \"Competitive Positioning\", \"subtopics\": [{\"guid\": \"ec4aad22-060a-4687-bfbc-fea3d3a07571\", \"text\": \"Differentiation pillars\"}, {\"guid\": \"6658209a-76c5-4a30-873b-1431901f36e1\", \"text\": \"Key competitors map\"}, {\"guid\": \"53a5ba03-3305-484d-a90a-fda4e6e4e6c5\", \"text\": \"Barrier strategies\"}]}]}, {\"guid\": \"194fd5de-5d8a-4a2b-abd4-7e4061810daa\", \"text\": \"Product & Tech\", \"subtopics\": [{\"guid\": \"c9fa1ccc-0f43-4157-a619-c1b551323f7b\", \"text\": \"MVP Design\", \"subtopics\": [{\"guid\": \"9a09ec78-dcbd-42ed-ad97-5b2fecd08f7d\", \"text\": \"Core feature set\", \"notes\": {\"text\": \"Some Notes\"}}, {\"guid\": \"4697cf4f-b2b8-4e79-9ccc-b18841a8756e\", \"text\": \"User flows map\", \"links\": [{\"text\": \"label\", \"url\": \"https://example.com\"}]}, {\"guid\": \"0e73917c-32f6-471f-abf3-8e4860fa9420\", \"text\": \"Rapid prototyping\", \"references\": [{\"guid_1\": \"0e73917c-32f6-471f-abf3-8e4860fa9420\", \"guid_2\": \"ccdb5ce8-e25d-4633-8b89-f052b5f5d034\", \"direction\": 1}]}]}]}]"})
...
on doWriteTree(jsonData)
	try
		...
		set guidMapping to {}
		set pendingReferences to {}
		set guidMappingDict to NSMutableDictionary's alloc()'s init()
		...
		tell application "MindManager"
			...
			my _registerTopicGuid(topicGuid of parsedData, id of rootTopic)
			my _queueReferences(topicReferences of parsedData)
			
			-- Build the tree structure recursively
			set result to my buildTreeFromJSON(rootTopic, topicSubtopics of parsedData)
			if result contains "error" then
				return result
			end if
		end tell
		
		set relResult to my _processPendingReferences()
		if relResult contains "error" then
			return relResult
		end if
		...
		tell application "MindManager"
			tell document 1
				...
				set newSubtopic to make new subtopic at end of subtopics of targetTopic
					...
					my _registerTopicGuid(topicGuid of parsedData, id of newSubtopic)
					my _queueReferences(topicReferences of parsedData)
			...
			tell application "MindManager"
				tell parentTopic
					set newSubtopic to make new subtopic at end of subtopics
						...
						my _registerTopicGuid(topicGuid of parsedSubtopic, id of newSubtopic)
						my _queueReferences(topicReferences of parsedSubtopic)
				end tell
			end tell
...
on _registerTopicGuid(originalGuid, mmGuid)
	if originalGuid is "" then return
	if mmGuid is missing value then return
	try
		set end of guidMapping to {originalGuid, mmGuid}
		if guidMappingDict is not missing value then
			(guidMappingDict's setObject:mmGuid forKey:originalGuid)
		end if
	on error recordErr number recordErrNum
		log "Failed to register mapping for GUID " & originalGuid & ": " & (recordErr as text) & " (" & recordErrNum & ")"
	end try
end _registerTopicGuid

on _lookupMappedID(originalGuid)
	if originalGuid is "" then return ""
	try
		if guidMappingDict is not missing value then
			set found to guidMappingDict's objectForKey:originalGuid
			if found is not missing value then return found as text
		end if
	on error
		-- fall back to linear scan
	end try
	repeat with pair in guidMapping
		try
			if (item 1 of pair) is originalGuid then return item 2 of pair
		end try
	end repeat
	return ""
end _lookupMappedID

on _queueReferences(refList)
	if refList is missing value then return
	if class of refList is not list then return
	repeat with refEntry in refList
		try
			set refGuid1 to item 1 of refEntry
		on error
			set refGuid1 to ""
		end try
		try
			set refGuid2 to item 2 of refEntry
		on error
			set refGuid2 to ""
		end try
		try
			set refDir to item 3 of refEntry
		on error
			set refDir to 0
		end try
		set end of pendingReferences to {refGuid1, refGuid2, refDir}
	end repeat
end _queueReferences

on _processPendingReferences()
	try
		tell application "MindManager"
			repeat with refEntry in pendingReferences
				set refGuid1 to ""
				set refGuid2 to ""
				set refDirection to 0
				try
					set refGuid1 to item 1 of refEntry
				end try
				try
				...
				try
					set refDirection to item 3 of refEntry
				end try
				
				set startID to my _lookupMappedID(refGuid1)
				set endID to my _lookupMappedID(refGuid2)
				if startID is "" or endID is "" then
					log "Skipping relationship; missing mapped IDs for " & refGuid1 & " or " & refGuid2
				else
					set t1 to my _findTopicByID(startID)
					set t2 to my _findTopicByID(endID)
					if t1 is not missing value and t2 is not missing value then
						try
							set dirInt to 0
							try
								set dirInt to refDirection as integer
							end try
							if dirInt is -1 then
								make new relationship with properties {{starting location:t2, ending location:t1}}
							else
								make new relationship with properties {{starting location:t1, ending location:t2}}
							end if
						on error relErr number relErrNum
							log "Failed to create relationship for " & refGuid1 & " -> " & refGuid2 & ": " & (relErr as text) & " (" & relErrNum & ")"
						end try
					end if
				end if
			end repeat
		...
		return "success"
	on error errMsg number errNum
		return "{ \"error\": \"Error creating relationships: " & my escapeJSON(errMsg) & " (Code: " & errNum & ")\" }"
	end try
end _processPendingReferences
	...
	try
		...
		if jsonData is missing value then error "Unable to encode JSON text as UTF-8."
			...
			if parsedObject is missing value then
				...
				return {errorStatus:true, errorMsg:errorText, topicGuid:"", topicText:"", topicLevel:0, topicNotes:"", topicReferences:{}, topicSubtopics:{}}
			end if
			
			if ((parsedObject's isKindOfClass:NSDictionary) as boolean) is false then
			return {errorStatus:true, errorMsg:"JSON root is not an object.", topicGuid:"", topicText:"", topicLevel:0, topicNotes:"", topicReferences:{}, topicSubtopics:{}}
		end if
		
		return my _dictionaryToTopicRecord(parsedObject)
		
	on error errMsg number errNum
		return {errorStatus:true, errorMsg:"JSON parsing error: " & errMsg & " (Code: " & errNum & ")", topicGuid:"", topicText:"", topicLevel:0, topicNotes:"", topicReferences:{}, topicSubtopics:{}}
	end try
end parseTopicJSON
...
on _dictionaryToTopicRecord(topicDict)
	...
	-- Collect references for later relationship creation
	set referenceRecords to {}
	set rawReferences to (topicDict's objectForKey_("references"))
	if rawReferences is not missing value then
		try
			if ((rawReferences's isKindOfClass:NSArray) as boolean) is true then
				set referencesCount to ((rawReferences's |count|()) as integer)
				if referencesCount > 0 then
					repeat with i from 0 to (referencesCount - 1)
						set refDict to (rawReferences's objectAtIndex:i)
						if ((refDict's isKindOfClass:NSDictionary) as boolean) is true then
							set refGuid1 to my _coerceToText((refDict's objectForKey_("guid_1")))
							set refGuid2 to my _coerceToText((refDict's objectForKey_("guid_2")))
							set rawDirection to (refDict's objectForKey_("direction"))
							set refDirection to 0
							if rawDirection is not missing value then
								try
									if ((rawDirection's isKindOfClass:NSNull) as boolean) is false then
										set refDirection to rawDirection as integer
									end if
								end try
							end if
							set end of referenceRecords to {refGuid1, refGuid2, refDirection}
						end if
					end repeat
				end if
			end if
	...
	return {errorStatus:false, errorMsg:"", topicGuid:topicGuid, topicText:topicText, topicLevel:topicLevel, topicNotes:topicNotes, topicReferences:referenceRecords, topicSubtopics:subtopicRecords}
